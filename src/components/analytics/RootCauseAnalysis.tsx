...
import { Bug, BookOpen, Lightbulb, CreditCard, Plug, Settings, Zap, Database, HelpCircle, TrendingUp } from "lucide-react";
...
export default function RootCauseAnalysis() {
  ...
  const { data, isLoading, error, refetch } = useQuery<RootCauseData>({
    queryKey: ['root-cause', timeRange],
    queryFn: async () => {
      const token = await getAccessToken();
      const response = await fetch(`/api/analytics?type=root-cause&days=${timeRange}`, {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-lg">Analyzing root causes with AI...</div>
      </div>
    );
  }
